...
import {
  ...
  type QuestionData,
  type OptionData,
} from "../new/components/QuestionEditor"; // Reuse component
...
export default function EditQuizPage() {
  ...
  useEffect(() => {
    if (quiz) {
      ...
      setAntiTabSwitchEnabled((quiz as any).antiTabSwitchEnabled ?? false);
      setQuestions(
  ...
  if (!quiz) {
    return <div className="py-20 text-center">Quiz not found</div>;
  }
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="text-muted-foreground h-9 w-9 hover:bg-rose-500/10 hover:text-rose-500"
            onClick={async () => {
              if (
                await confirm(
