...
export async function getSessionGameState(
    ...
    if (currentQuestion) {
      ...
      if (canonicalQ) {
        const idx = session.quiz.questions.findIndex(
          (q: { id: string }) => q.id === canonicalQ.id,
        );
      ...
      if (session.status === "ACTIVE") {
        effectiveStartTime =
          session.currentQuestionStartTime?.toISOString() ??
  ...
  const isHistory =
    currentQuestion &&
    currentQuestion.order < (session.highestQuestionOrder || 0);
  ...
  if (currentQuestion) {
    ...
    answers.forEach(
      (a: {
        ...
        _count: { selectedOptionId: number; _all: number };
      }) => {
    ...
    if (session.status === "INTERMISSION") {
      ...
      answers.forEach(
        (a: {
          selectedOptionId: string | null;
          _count: { selectedOptionId: number };
        }) => {
  ...
  return {
    ...
    antiTabSwitchEnabled: session.quiz.antiTabSwitchEnabled ?? false,
    answersCount,
  };
}
...
export const gameRouter = createTRPCRouter({
  joinSession: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      console.log(
        `Player joined: ${player.name} (${player.id}) to session ${session.id} status ${session.status} `,
      );
      ...
      const personalState = await getSessionGameState(
        ctx.db,
        session.id,
        player.id,
      );
  ...
  onSessionUpdate: publicProcedure
    ...
    .subscription(async function* ({ ctx, input }) {
      console.log(
        `[SUBSCRIPTION - START] sessionId = ${input.sessionId} time = ${new Date().toISOString()} `,
      );
      ...
      try {
        while (!shouldStop) {
          ...
          const msUntilNextSlot =
            currentPollInterval - (now % currentPollInterval);
        ...
        console.log(
          `[SUBSCRIPTION - END] sessionId = ${input.sessionId} time = ${new Date().toISOString()} `,
        );
      }
    }),
  ...
  submitAnswer: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const question = session.quiz.questions.find(
        (q: { id: string }) => q.id === input.questionId,
      );
      if (!question)
        throw new TRPCError({
          code: "NOT_FOUND",
      ...
      if (session.currentQuestionId && session.quiz.shuffleQuestions) {
        const currentCanonical = session.quiz.questions.find(
        ...
        if (currentCanonical && question.order === currentCanonical.order) {
          ...
          const assignedQuestion = getShuffledQuestion(
            session.quiz.questions,
            currentCanonical,
            input.playerId,
            session.id,
          ...
          if (assignedQuestion.id !== question.id) {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "Invalid question for this player",
            });
          }
        }
      }
      ...
      try {
        await pusher.trigger(`session-${input.sessionId}`, "answer-submitted", {
  ...
  reportTabEvent: publicProcedure
    .input(
      z.object({
        sessionId: z.string(),
        playerId: z.string(),
    ...
    .mutation(async ({ ctx, input }) => {
      const session = await ctx.db.gameSession.findUnique({
        where: { id: input.sessionId },
        include: {
          quiz: { select: { antiTabSwitchEnabled: true } },
          players: { select: { id: true, name: true, class: true } },
        },
      });

      if (!session || !session.quiz.antiTabSwitchEnabled) {
        return { ignored: true };
      }

      const player = session.players.find((p) => p.id === input.playerId);

      const event = await ctx.db.tabEvent.create({
        data: {
          sessionId: input.sessionId,
          playerId: player?.id,
          eventType: input.state,
          details: {
      ...
      const payload = {
        id: event.id,
        playerId: player?.id ?? input.playerId,
      ...
      try {
        ...
        console.error("Failed to trigger tab event update:", e);
      }

      return { success: true };
    }),

  getTabEvents: protectedProcedure
    .input(
      z.object({
        sessionId: z.string(),
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!session || session.quiz.createdById !== ctx.session.user.id) {
        throw new TRPCError({
          code: "FORBIDDEN",
      ...
      const events = await ctx.db.tabEvent.findMany({
      ...
      return events.map((e) => ({
        ...
        playerClass: e.player?.class ?? undefined,
